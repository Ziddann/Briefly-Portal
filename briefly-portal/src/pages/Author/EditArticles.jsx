...
function EditNews() {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/author/news/${id}`)
      .then(res => res.json())
      .then(data => {
        setForm({
          title: data.title,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/author/news/${id}`, {
        method: 'PUT',
        body: formData,
      });
  ...
  return (
    <div className="edit-news-container">
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        {preview && <img src={preview.startsWith('http') ? preview : `http://localhost:5000${preview}`} alt="Preview" className="preview-img" />}
